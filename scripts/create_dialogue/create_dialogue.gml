...
function create_dialogue() {
	...
	with(_textbox){
		...
		//Speaker's Variables
		i = 0; repeat(text_len){
			if(variable_instance_exists(_speaker[i], "myPortrait"))		{ portrait[i] = _speaker[i].myPortrait; }
			else { portrait[i] = -1; }
			if(variable_instance_exists(_speaker[i], "myVoice"))		{ voice[i] = _speaker[i].myVoice; }
			else { voice[i] = -1; }
			if(variable_instance_exists(_speaker[i], "myFont"))		{ font[i] = _speaker[i].myFont; }
			else { font[i] = font_reg; }
			if(variable_instance_exists(_speaker[i], "myName"))		{ name[i] = _speaker[i].myName; }
			else { name[i] = "None"; }
