...
if ( min_draw > 0 ) {
	draw_sprite(s_carat_pink_up, 0, gui_width / 2 - 80, 45);	
}
if ( max_draw < ds_list_size(options) - 1 ) {
	draw_sprite(s_carat_pink_down, 0, gui_width / 2 - 80, 200);	
}
...
for ( var i = min_draw; i <= max_draw; i++ ) {
	if ( i >= 0 && i < ds_list_size(options) ) {
		...
		var isDiscovered = o_game.friends_discovered[| i];
		...
		if ( device_mouse_x_to_gui(0) == clamp(device_mouse_x_to_gui(0), x + 16, x + 100) && device_mouse_y_to_gui(0) == clamp(device_mouse_y_to_gui(0),  70 + quest_height * clamp_index,  70 + quest_height * clamp_index + quest_height) ) {
			mouse_hover_index = i;
			draw_text_ext_transformed_color(x+25, 70 + quest_height * clamp_index, text, 3, 400, 0.7, 0.7, 0, text_color_active, text_color_active, text_color_active, text_color_active, 1);
			...
			draw_text_ext_transformed_color(x+25, 70 + quest_height * clamp_index, text, 3, 400, 0.7, 0.7, 0, color, color, color, color, 1);
		}
	}
	clamp_index++;
}
...
draw_sprite(s_ui_caret, 0, x + 15,70 + quest_height * selected_clamp_index);	