...
draw_sprite_ext(s_items_bg, 0, gui_width / 2, 55, 1, 1, 0, c_white, 1);
if ( option_grid == 0 ) {
	draw_set_font(font_reg);
	draw_set_halign(fa_center);
	draw_text_ext_transformed(x + 80, y + 50, "You don't have any items!", -1, 200, 0.5, 0.5, 0);
	...
	for ( var i = 0 + minRowtoShow; i < actualRows + minRowtoShow; i++ ) {
		for ( var j = 0; j < numCols; j++ ) {
			if ( option_grid[i, j] != -1 ) {
				...
				var item = o_game.items[| item_slot];
				var item_count = o_game.item_number[| item_slot];
				...
				draw_text_ext_transformed_color(x + j * 35 + 25, y + displayi * 35 + 17, item_count, 0, 100, 0.3, 0.3, 0, c_black, c_black, c_black, c_black, 1);
				draw_set_halign(fa_left);
