...
//Draw portrait
if(portrait[page] != -1){
	draw_sprite_ext(portrait[page], emotion[page], pos_x-portraitWidth + 20, portrait_pos_y, scale,scale, 0, c_white, 1);

	#region Idle Animated Portrait
if(type[page] == 1 or charCount >= str_len) {
	if(portrait_idle[page] != -1){
		var posx = pos_x-portraitWidth; var posy = portrait_pos_y;
...
if(cname != "None"){
	...
	//Draw namebox
	draw_nine_slice(name_box, name_box_x,name_box_y - 8 * scale, name_box_x + string_width(cname) + 16, name_box_y + string_height(cname));
	
	//Draw name text
	c = name_col;
	draw_set_halign(fa_left);
	draw_text_color(name_box_x + 8, name_box_text_y - 10, cname, c,c,c,c, 1);
}
#endregion
